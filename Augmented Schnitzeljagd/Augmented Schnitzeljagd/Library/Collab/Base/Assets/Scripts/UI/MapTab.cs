...
public class MapTab : AbstractTab
{
    ...
    void Start()
    {
        ...
        else
        {
            ...
            route = (Route)GameData.Instance.RequestServer(Strings.REQUEST_ROUTE + ":" + Strings.ROUTENAME + "=" + GameData.Instance.Me.CurrentRoute);
            // Playing Route
        ...
        if (!MenuOpened() && (!EventSystem.current.IsPointerOverGameObject(0) || !EventSystem.current.IsPointerOverGameObject()))
        {
            ...
            if (Physics.Raycast(ray, out hit, 100))
            {
                ...
                if (hit.transform.gameObject.layer == 10)
                {
                    ...
                    route = (Route)GameData.Instance.RequestServer(Strings.REQUEST_ROUTE + ":" + Strings.ROUTENAME + "=" + routeName);
    ...
    //---------------------------------------------------------------------------------------------//
    public void GetNearbyRoutesButton()
    {
        //OtherUsers
        ...
        string response = GameData.Instance.RequestUser(start: GameData.Instance.Me.Position, distance: 100);
        Debug.Log("UserList: " + response);
        if (!response.Equals(Strings.USER_NOT_FOUND))
        {
            StringList userList = (StringList)response;
        ...
        response = GameData.Instance.RequestServer(Strings.REQUEST_ROUTE + ":" + Strings.START_POSITION + "=" + GameData.Instance.Me.Position.Latitude + "," + GameData.Instance.Me.Position.Longitude + ";" + Strings.DISTANCE + "=" + 10);
        StringList routeList = new StringList();
        Debug.Log("RouteList: " + response);
        if (!response.Equals(Strings.ROUTE_NOT_FOUND))
        {
            routeList = (StringList)response;

            foreach (string r in routeList)
            {
                route = (Route)GameData.Instance.RequestServer(Strings.REQUEST_ROUTE + ":" + Strings.ROUTENAME + "=" + r);
                Debug.Log(route);
    ...
    public void CreateRoute()
    {
        ...
        if (nameField.text == "" || !GameData.Instance.RequestServer(Strings.REQUEST_ROUTE + ":" + Strings.ROUTENAME + "=" + nameField.text).Equals(Strings.ROUTE_NOT_FOUND))
        {
    ...
    {
        Debug.Log(GameData.Instance.RequestServer(Strings.CREATE_ROUTE + ":" + route));
