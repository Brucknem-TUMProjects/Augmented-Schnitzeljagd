...
public class GoogleStaticMap : MonoBehaviour {
	...
	private float overlap = 0.6f;
	public MapType mapType;

	// Max size for free access is 1024.
	public int horizontalSize = 1024;
	public int verticalSize = 1024;
	...
	public Vector2 getPositionOnMap(GeoPoint point) {
        try
        {
            Debug.Log("Setting point of " + this.name + " to: " + point.ToString());

            float delta_lon = point.lon_d - this._centerLatLon.lon_d;
            float delta_lat = point.lat_d - this._centerLatLon.lat_d;
        
	
		    return Vector2.Scale(new Vector2 (gameObject.transform.position.x / this.realWorldtoUnityWorldScale.x + this.mapRectangle.getWidthMeterLonRatio_deg () * delta_lon,
        ...
        {
            Debug.Log("Error in GoogleStaticMaps.getPositionOnMap() --> " + e.Message);
            return Vector2.zero;
        }
    }
